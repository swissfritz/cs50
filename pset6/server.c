...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            for (int i = 0, n = needle - haystack + 2; i < n; i++)
            {
                if (strcmp(&line[i], " ") == 0)
                    space++;
            ...
            if (space != 2 || slash < 1 || fstop < 1)
            {
                error(400);
                continue;
            }
                    
            // get strings for method, request-target and HTTP-version
            ...
            {
                first_tok = strtok(target, "?");
                query = strtok(NULL, "?");
            }

            // concatenate root and absolute-path
            char* path = strcat(root, first_tok);
            ...
            char* ext = strrchr(first_tok, '.');
            char exten[strlen(ext) + 1];
